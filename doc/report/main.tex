...
The rules of chess constrain the knight's transitions between timesteps. From a cell $(i, j)$ at timestep $t$, a knight moves in an ``L-shape'' to one of up to eight valid board positions at timestep $t+1$. We thus enforce the knight reaches one of these destinations. Let $L(i, j)$ denote the set of legal destination cells reachable from $(i, j)$. Formally,
...
To improve unit propagation in the SAT solver, we can additionally enforce the converse constraints. That is, if a cell $(i', j')$ is occupied at timestep $t+1$, the knight must have arrived from a valid preceding cell at timestep $t$. Let $L'(i', j')$ denote the set of legal origin cells that can reach $(i', j')$. Formally, 
